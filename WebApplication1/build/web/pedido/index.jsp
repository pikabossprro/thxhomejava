                         ...
                         <table id="datatable" class="table table-striped table-bordered nowrap py-2" style="width:100%" >
                             ...
                             <tbody>
                                 <c:forEach var="pedido" items="${lista}">
                                     <tr>
                                         <td><c:out value="${pedido.id_venta}"/></td>
                                         <td><c:out value="${pedido.forma_pago}" /></td>
                                         <td><c:out value="${pedido.metodo_entrega}" /></td>
                                         <td><c:out value="${pedido.total}" /></td>
                                         <td><c:out value="${pedido.apellido}"/></td>
                                         <td><c:out value="${pedido.nombre}"/></td>
                                         <td>
                                             <form action="pedidoController?accion=guardar" method="post"class="text-center">
                                                 <input type="hidden" name="id" value="<c:out  value="${pedido.id_venta}" />">
                                             <select name="estado_pedido"  class="form-select"  >
                                                 <option value="1" ><c:out  value="${pedido.estado_pedido}" /></option>
                                                 <option value="2" >Entregado</option>  
                                             </select>
                                                    <button type="submit" class="btn btn-success py-2"><i class="fa-solid fa-pen-to-square"></i></button>
                                             </form>   
                                         </td>
                                         <td>
                                             <a  href="pedidoController?accion=ver&id=<c:out  value="${pedido.id_venta}" />" class="btn btn-primary"><i class="fa-solid fa-eye"></i></a>
                                             <a  href="pedidoController?accion=eliminar&id=<c:out  value="${pedido.id_venta}" />"  class="btn btn-danger"><i class="fa-solid fa-trash"></i></a>                                           
                                         </td>
                                     </tr>
                                 </c:forEach>    
                             </tbody>
